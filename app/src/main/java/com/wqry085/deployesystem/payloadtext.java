...
public class payloadtext {
     ...
     * 获取配置的详细信息（用于调试）
     */
    public static String getConfigInfo(Context context, String configText) {
        if (configText == null) return context.getString(R.string.cfg_null);

        String[] lines = configText.split("\\r?\\n");
        StringBuilder info = new StringBuilder();
        info.append(context.getString(R.string.cfg_total_lines, lines.length));
        ...
        for (int i = 0; i < lines.length; i++) {
            if (i == 0) {
                info.append(context.getString(R.string.cfg_counter_line, lines[i]));
            } else if (lines[i].trim().isEmpty()) {
                emptyLineCount++;
                info.append(context.getString(R.string.cfg_empty_line, emptyLineCount, i));
            } else {
                paramCount++;
                info.append(context.getString(R.string.cfg_param_line, paramCount, lines[i]));
            }
        }

        info.append(context.getString(R.string.cfg_non_empty_summary, paramCount));
        info.append(context.getString(R.string.cfg_empty_summary, emptyLineCount));
